...
def send_to_telegram(
    ...
    space = "\n" * 2
    telegram_chat_id = frappe.db.get_value(
        "Telegram User Settings", telegram_user, "telegram_chat_id"
    )
    telegram_settings = frappe.db.get_value(
        "Telegram User Settings", telegram_user, "telegram_settings"
    )
    telegram_token = frappe.db.get_value(
        "Telegram Settings", telegram_settings, "telegram_token"
    )
			...
			if loop and loop.is_running():
				async def send_async():
					with open(file_path, "rb") as photo_file:
						try:
							send_photo_response = await bot.send_photo(
							...
							if meta.has_field("telegram_notification_id"):
								frappe.db.set_value(reference_doctype, reference_name, "telegram_notification_id", send_photo_response.message_id)
								frappe.db.commit()
							...
							await bot.send_message(
								chat_id=telegram_chat_id, text=f"{message}\n {e}"
							)
